...
DynamoDB doesn't assign a separate resource ARN to each key in a table, but
you can write more precise policies for a DynamoDB table
[using an IAM `Condition` element](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/specifying-conditions.html).
For example, you can use the `dynamodb:LeadingKeys` condition key to match
on the partition key values that the S3 backend will use:
...
In the above, `TableName` represents the table name given in the
`dynamodb_table` argument in the backend configuration, and
`myapp/production/tfstate` represents the path to the particular S3 object
for this environment's state. DynamoDB ARNs are regional and account-specific,
unlike S3 bucket ARNs, so be sure to also specify the correct region and
AWS account ID for your DynamoDB table in the `Resource` element.
